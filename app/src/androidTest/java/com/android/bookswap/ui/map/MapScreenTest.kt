...
class MapScreenTest {
  ...
  fun mapHasGeoLocation() {
    ...
    assertEquals(
        geolocation.userLocation.value.latitude, cameraPositionState?.position?.target?.latitude)
    assertEquals(
        geolocation.userLocation.value.longitude, cameraPositionState?.position?.target?.longitude)
  }
}
