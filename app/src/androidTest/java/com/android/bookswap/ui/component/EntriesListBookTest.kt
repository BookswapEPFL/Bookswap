...
class EntriesListBookTest {
    ...
    fun testRatingFieldAcceptsValidInput() {
        ...
        // Input valid rating
        composeTestRule.onNodeWithTag(C.Tag.BookEntryComp.rating_star_empty + "_4",useUnmergedTree = true).performClick()
        composeTestRule.onNodeWithTag(C.Tag.BookEntryComp.rating_star + "_4",useUnmergedTree = true).assertExists()

        // Input invalid rating
        composeTestRule.onNodeWithTag(C.Tag.BookEntryComp.rating_star_empty + "_5",useUnmergedTree = true).assertExists()
    ...
    fun testLanguageDropdownSelection() {
        ...
        composeTestRule.onNodeWithTag("${C.Tag.BookEntryComp.language_menu}_$languageToSelect").performClick()
    ...
    fun testHasInitialValue(){
        ...
        composeTestRule.onNodeWithTag(C.Tag.BookEntryComp.genre_field).assertTextContains(BookGenres.FICTION.Genre)
        composeTestRule.onNodeWithTag(C.Tag.BookEntryComp.author_field).assertTextContains("Author")
        composeTestRule.onNodeWithTag(C.Tag.BookEntryComp.description_field).assertTextContains("Description")
        composeTestRule.onNodeWithTag(C.Tag.BookEntryComp.rating_star + "_5", useUnmergedTree = true).assertExists()
        composeTestRule.onNodeWithTag(C.Tag.BookEntryComp.isbn_field).assertTextContains("1234567890")
        composeTestRule.onNodeWithTag(C.Tag.BookEntryComp.photo_field).assertTextContains("https://example.com/photo.jpg")
