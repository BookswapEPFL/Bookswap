...
fun UserProfile(userVM: UserViewModel, modifier: Modifier = Modifier) {
    ...
    if (showEditProfile) {
        EditProfileDialog(
            ...
            onSave = {
                userVM.updateUser(greeting = it.greeting, firstName = it.firstName, lastName = it.lastName, email = it.email, phone = it.phoneNumber)
                showEditProfile = false
                needRecompose = true
            },
            user)
    ...
    BookSwapAppTheme() {
        ...
        Scaffold(
            modifier = mod
                .testTag("profileScreenContainer")
                .padding(0.dp),
            topBar = {
                TopAppBar(modifier = mod,
                    // Title of the screen
                    title = {
                        Text(
                            "Your Profile",
                            modifier = mod
                                .testTag("profileTitleText")
                        )
                    },
                    // Icon button for navigation (currently no action defined)
                    navigationIcon = {
                        IconButton(modifier = mod
                            .size(50.dp)
                            ,onClick = {}) {
                            Column(modifier = mod.fillMaxSize(), Arrangement.Center, Alignment.CenterHorizontally) {

                                Text(text = "",modifier = mod
                ...
                Row(modifier = mod
                    ...
                    Column(
                        ...
                        Button(
                            ...
                            enabled = true,
                            modifier = mod
                                .testTag("editProfileBtn")
                        ) {
                            // Text displayed on the button
                            Text("Edit Profile",
                                modifier = mod
                                    .testTag("editProfileBtnTxt"))
                        }
                    }
                }
            })
