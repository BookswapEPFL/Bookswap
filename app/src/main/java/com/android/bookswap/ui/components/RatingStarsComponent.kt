    ...
    for (i in 1..5) {
      IconButton(onClick = { onRatingChanged(i) }) {
        Icon(
            ...
            tint = if (i <= currentRating) ColorVariable.Accent else emptyStarColor,
            modifier =
                Modifier.testTag(
                    if (i <= currentRating) C.Tag.BookEntryComp.rating_star + "_$i"
                    else C.Tag.BookEntryComp.rating_star_empty + "_$i"))
      }
    }
  }
}
