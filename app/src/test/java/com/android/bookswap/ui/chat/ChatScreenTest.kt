...
class ChatScreenTest {
    ...
    fun hasRequiredComponentsAndShowsMessages() {
        `when`(mockMessageRepository.addMessagesListener(any(), any(), any()))
        ...
        placeHolderData.forEach { message ->
            ...
            composeTestRule.onNodeWithTag("message_text ${message.id}")
                .assertTextEquals(message.text)
            composeTestRule.onNodeWithTag("message_timestamp ${message.id}").assertIsDisplayed()
            composeTestRule.onNodeWithTag("message_timestamp ${message.id}")
